...
describe("IVMS101 Fast-Check Property-Based Tests", () => {
	describe("Basic Type Generation", () => {
		it("should generate valid natural person name types", () => {
			fc.assert(
				fc.property(arb.naturalPersonNameTypeCode(), (nameType) => {
					const validTypes: IVMS101_2020.NaturalPersonNameTypeCode[] = [
						"ALIA",
						"BIRT",
						"MAID",
						"LEGL",
		...
		it("should generate valid legal person name types", () => {
			fc.assert(
				fc.property(arb.legalPersonNameTypeCode(), (nameType) => {
					const validTypes: IVMS101_2020.LegalPersonNameTypeCode[] = [
						"LEGL",
						"SHRT",
						"TRAD",
					];
		...
		it("should generate valid address types", () => {
			fc.assert(
				fc.property(arb.addressTypeCode(), (addressType) => {
					const validTypes: IVMS101_2020.AddressTypeCode[] = [
						"HOME",
						"BIZZ",
						"GEOG",
					];
	...
	describe("Structure Generation", () => {
		it("should generate valid natural person name IDs", () => {
			fc.assert(
				fc.property(arb.naturalPersonNameId(), (nameId) => {
		...
		it("should generate valid legal person name IDs", () => {
			fc.assert(
				fc.property(arb.legalPersonNameId(), (nameId) => {
		...
		it("should roundtrip conversion preserve essential data", () => {
			fc.assert(
				fc.property(arb.ivms101_2020(), (original) => {
					...
					expect(ivms101_version(original)).toBe(
						IVMS101_2023.PayloadVersionCode.V2020,
					);
					expect(ivms101_version(converted2023)).toBe(
					...
					original.originator.originatorPersons.forEach((person, idx) => {
						if (person.naturalPerson) {
							person.naturalPerson.name.nameIdentifier.forEach(
								(nameId, nameIdx) => {
									const convertedName =
										converted2023.originator.originatorPerson[idx].naturalPerson
											?.name.nameIdentifier[nameIdx];
									expect(convertedName?.primaryIdentifier).toBe(
										nameId.primaryIdentifier,
									);
									expect(convertedName?.secondaryIdentifier).toBe(
										nameId.secondaryIdentifier,
									);
									// Note: nameIdentifierType becomes naturalPersonNameIdentifierType in 2023
									expect(convertedName?.naturalPersonNameIdentifierType).toBe(
										nameId.nameIdentifierType,
									);
								},
							);
				...
				fc.property(arb.ivms101_2020(), (original) => {
					...
					original.originator.originatorPersons.forEach((person, idx) => {
						...
						if (person.legalPerson?.customerNumber) {
							expect(
								converted2023.originator.originatorPerson[idx].legalPerson
									?.customerIdentification,
	...
	describe("IVMS101 2023 Structure Generation", () => {
		it("should generate valid IVMS101 2023 structures", () => {
			fc.assert(
				fc.property(arb.ivms101_2023Valid(), (ivms) => {
					...
					expect(ivms.payloadMetadata?.payloadVersion).toBe(
						IVMS101_2023.PayloadVersionCode.V2023,
					);
				}),
			);
		...
		it("should generate valid 2023 natural person name identifiers", () => {
			fc.assert(
				fc.property(arb.naturalPersonNameId2023(), (nameId) => {
					expect(nameId.primaryIdentifier).toBeTypeOf("string");
					expect(nameId.primaryIdentifier.length).toBeGreaterThan(0);
					expect(["ALIA", "BIRT", "MAID", "LEGL", "MISC"]).toContain(
						nameId.naturalPersonNameIdentifierType,
					);
					if (nameId.secondaryIdentifier !== undefined) {
						expect(nameId.secondaryIdentifier).toBeTypeOf("string");
					}
		...
		it("should use customerIdentification instead of customerNumber in 2023", () => {
			fc.assert(
				fc.property(arb.ivms101_2023Valid(), (ivms) => {
					[
						...ivms.originator.originatorPerson,
						...ivms.beneficiary.beneficiaryPerson,
					].forEach((person) => {
						if (person.naturalPerson) {
							// Should not have customerNumber (2020 field)
							expect(
								(person.naturalPerson as Record<string, unknown>)
									.customerNumber,
							).toBeUndefined();
							// May have customerIdentification (2023 field)
							if (person.naturalPerson.customerIdentification !== undefined) {
								expect(person.naturalPerson.customerIdentification).toBeTypeOf(
									"string",
								);
							}
						}
						if (person.legalPerson) {
							// Should not have customerNumber (2020 field)
							expect(
								(person.legalPerson as Record<string, unknown>).customerNumber,
							).toBeUndefined();
							// May have customerIdentification (2023 field)
							if (person.legalPerson.customerIdentification !== undefined) {
								expect(person.legalPerson.customerIdentification).toBeTypeOf(
									"string",
								);
							}
						}
	...
	describe("Cross-Version Compatibility Tests", () => {
		it("should handle conversion from 2020 to 2023 structure", () => {
			fc.assert(
				fc.property(arb.ivms101_2020(), (original2020) => {
					...
					expect(
						(converted2023.originator as Record<string, unknown>)
							.originatorPersons,
					).toBeUndefined();
					expect(
						(converted2023.beneficiary as Record<string, unknown>)
							.beneficiaryPersons,
					).toBeUndefined();

					// Array lengths should match
					expect(converted2023.originator.originatorPerson.length).toBe(
						original2020.originator.originatorPersons.length,
					);
					expect(converted2023.beneficiary.beneficiaryPerson.length).toBe(
						original2020.beneficiary.beneficiaryPersons.length,
					);
				}),
			);
		});
		...
		it("should maintain referential integrity across conversions", () => {
			fc.assert(
				fc.property(arb.ivms101_2020(), (original) => {
					...
					expect(backConverted.originator.accountNumber).toEqual(
						original.originator.accountNumber,
					);
					expect(backConverted.beneficiary.accountNumber).toEqual(
						original.beneficiary.accountNumber,
					);
					...
					original.originator.originatorPersons.forEach((person, idx) => {
						if (person.naturalPerson?.geographicAddress) {
							expect(
								backConverted.originator.originatorPersons[idx].naturalPerson
									?.geographicAddress,
		...
		it("should maintain data type consistency in 2020 format", () => {
			fc.assert(
				fc.property(arb.ivms101_2020(), (ivms) => {
					...
					[
						...ivms.originator.originatorPersons,
						...ivms.beneficiary.beneficiaryPersons,
					].forEach((person) => {
						if (person.naturalPerson) {
							expect(
						...
						if (person.legalPerson) {
							expect(
								person.legalPerson.name.nameIdentifier.length,
							).toBeGreaterThan(0);
						}
					});
					...
					[
						...
						if (person.naturalPerson?.dateAndPlaceOfBirth) {
							const date = new Date(
								person.naturalPerson.dateAndPlaceOfBirth.dateOfBirth,
							);
							expect(date.getTime()).not.toBeNaN();
							expect(
								person.naturalPerson.dateAndPlaceOfBirth.placeOfBirth.length,
							).toBeGreaterThan(0);
						}
					});
				}),
				...
				fc.property(arb.naturalPerson(), (person) => {
					if (person.nationalIdentification) {
						...
						expect(validNaturalPersonTypes).toContain(
							person.nationalIdentification.nationalIdentifierType,
						);
					}
				}),
			);
			...
			fc.assert(
				fc.property(arb.legalPerson(), (person) => {
					if (person.nationalIdentification) {
						...
						expect(validLegalEntityTypes).toContain(
							person.nationalIdentification.nationalIdentifierType,
						);
					}
				}),
