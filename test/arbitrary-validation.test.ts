...
describe("Comprehensive Arbitrary Validation", () => {
	...
	describe("Exhaustive IVMS101 2020 Validation", () => {
		it("should generate 100% valid IVMS101 2020 structures", () => {
			let validCount = 0;
			let invalidCount = 0;
			const errors: Array<{ data: unknown; error: unknown }> = [];
			...
			if (invalidCount > 0) {
				console.error(
					`Found ${invalidCount} invalid samples out of ${TEST_SAMPLES}`,
				);
		...
		it("should generate structures with diverse data patterns", () => {
			...
			const hasOptionalFields = samples.filter(
				(s) => s.originatingVASP || s.beneficiaryVASP || s.transferPath,
			);
			const hasAccountNumbers = samples.filter(
				(s) =>
					...
					ivms.originator.originatorPerson.forEach((person) => {
						if (person.naturalPerson) {
							...
							if (
								...
								expect(person.naturalPerson).toHaveProperty(
						...
						if (person.legalPerson) {
							...
							if (
								(person.legalPerson as Record<string, unknown>)
									.customerIdentification !== undefined
							) {
								expect(person.legalPerson).toHaveProperty(
									"customerIdentification",
								);
							}
						}
		...
		it("should ensure arrays have minimum required length", () => {
			...
			fc.assert(
				fc.property(arb.beneficiary(), (beneficiary) => {
					expect(beneficiary.beneficiaryPersons.length).toBeGreaterThanOrEqual(
						1,
					);
					return true;
				}),
	...
	describe("Edge Cases and Boundary Testing", () => {
		it("should handle optional fields correctly", () => {
			...
			const withOptional2020 = samples2020.filter(
				(s) =>
					s.originatingVASP ||
					s.beneficiaryVASP ||
					s.transferPath ||
			...
			const withOriginatingVASP = samples2020.filter(
				(s) => s.originatingVASP,
			).length;
			const withBeneficiaryVASP = samples2020.filter(
				(s) => s.beneficiaryVASP,
			).length;
			const _withTransferPath = samples2020.filter(
				(s) => s.transferPath,
			).length;
			const _withPayloadMetadata = samples2020.filter(
				(s) => s.payloadMetadata,
			).length;
				...
				fc.property(arb.ivms101_2020(), (ivms) => {
					...
					[
						...
						if (person.naturalPerson) {
							...
							person.naturalPerson.name.nameIdentifier.forEach((nameId) => {
								expect(nameId.primaryIdentifier.trim().length).toBeGreaterThan(
									0,
								);
							});
						}
	...
	describe("Statistical Analysis", () => {
		...
		it("should generate diverse optional field combinations", () => {
			...
			const _withDOB = samples.filter((p) => p.dateAndPlaceOfBirth).length;
			const _withCountryRes = samples.filter(
				(p) => p.countryOfResidence,
			).length;
			const _withGeoAddr = samples.filter((p) => p.geographicAddress).length;
